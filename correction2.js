...
let choix = ""; // Choix vide ici pour pas que le chois soit undefined dans la boucle
...
function buyProduct(price, credit) {
  ...
  if (credit >= price) {
    ...
    return credit; // On renvoi le nouveau crédit
  } else {
...
// POIT VIRGULE A CHAQUE FIN D'INSTRUCTION EN JS;

// TANT QUE le crédit est supérieur ou égal à la BNA (5) et que le choix est différent de 4
while (credit >= bna && choix != 4) {
  // CREATION DES CONDITIONS

  let menu = menu1(credit);

  // Appel de la fonction menu avec pour paramètre crédit
  ...
  if (choix == 1) {
    ...
    credit = buyProduct(plat, credit);
  } else if (choix == 2) {
    // credit prend la valeur du return de la fonction buyProduct
    credit = buyProduct(bna, credit);
  } else if (choix == 3) {
    // credit prend la valeur du  return de la fonction buyProduct
    credit = buyProduct(ba, credit);
  } else {
    // Si le choix est == à 4 on envoi un message d'au revoir
    if (choix == 4) {
      alert("A bientôt");
    } else {
...
// FONCTION DE PRESENTATION AFFICHER UNE PHRASE AVEC TON NOM TON PRENOM TON METIER